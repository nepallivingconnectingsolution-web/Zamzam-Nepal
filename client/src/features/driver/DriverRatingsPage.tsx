...
const SERVICE_LABEL: Record<DriverReview["service"], string> = {
  taxi: "Taxi",
  bike: "Bike",
  parcel: "Parcel",
};
...
/**
 * The "driver admin" view of every rating a rider has left — same shape as
 * RestaurantReviewsPage / GroceryReviewsPage / HotelReviewsPage, just
 * reading GET /driver/reviews (+ /driver/reviews/summary) instead of a
 * partner-restaurant endpoint. Riders submit these from the "Rate this
 * trip" panel on /app/trips once their taxi/bike/parcel ride is COMPLETED,
 * so whatever service a rider used routes its review straight back to that
 * driver's own Ratings page here.
 */
export function DriverRatingsPage() {
  const summary = useResource<ReviewSummary>(() => api.get(endpoints.driver.reviewSummary), []);
  const reviews = useResource<DriverReview[]>(() => api.get(endpoints.driver.reviews), []);

  return (
    <div className="space-y-6">
      <PageHeader title="Ratings" subtitle="Rider feedback over time." />
      ...
      <AsyncBoundary
        ...
        empty={
          <EmptyState
            ...
            description="Once riders rate a completed taxi, bike or parcel trip, their feedback will show up here."
          />
        }
      >
            ...
            <Card key={r.id} className="p-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="accent">{SERVICE_LABEL[r.service]}</Badge>
                  <StarRating value={r.rating} size={15} />
                </div>
                <span className="text-xs text-muted-fg">{new Date(r.createdAt).toLocaleDateString()}</span>
              </div>
              <p className="mt-1 text-sm text-muted-fg">
                {r.from} → {r.to} • {new Date(r.rideDate).toLocaleDateString()}
              </p>

...
import type { PartnerRevenue } from "./types";

export function HotelRevenuePage() {
  const revenue = useResource<PartnerRevenue>(() => api.get(endpoints.hotels.partner.revenue), []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard label="Refunded" icon="AlertTriangle" state={revenue.state} value={s ? npr(s.totalRefunded, { compact: true }) : undefined} caption={s ? `${s.totalCancelled} cancelled booking(s)` : undefined} />
        <StatCard label="Avg. booking value" icon="Star" state={revenue.state} value={s ? npr(s.avgBookingValue, { compact: true }) : undefined} caption={s ? `${s.totalBookings} confirmed booking(s)` : undefined} />
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Daily net profit</CardTitle>
          <CardDescription>Last 30 days — the room revenue you keep, after excluding cancellations.</CardDescription>
        </CardHeader>
          ...
          <AsyncBoundary
            ...
            empty={
              <EmptyState
                ...
                description="This chart fills in as bookings come through."
              />
            }
          >
            <RevenueAreaChart
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Bookings, fees, and refunds by day — most recent first.</CardDescription>
        </CardHeader>
        <AsyncBoundary
          state={revenue.state}
          onRetry={revenue.refetch}
          label="Revenue"
          empty={<div className="px-5 pb-5"><EmptyState icon={<TrendingUp className="size-6" />} title="No bookings yet" /></div>}
        >

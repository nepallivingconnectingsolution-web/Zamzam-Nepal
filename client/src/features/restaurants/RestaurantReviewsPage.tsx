...
import type { FoodReviewSummary, PartnerFoodReview } from "./types";

export function RestaurantReviewsPage() {
  const summary = useResource<FoodReviewSummary>(() => api.get(endpoints.restaurants.partner.reviewSummary), []);
  const reviews = useResource<PartnerFoodReview[]>(() => api.get(endpoints.restaurants.partner.reviews), []);

  return (
    <div className="space-y-6">
      <PageHeader title="Reviews" subtitle="What customers are saying about your food." />
          ...
          {reviews.data?.map((r) => (
            <Card key={r.id} className="p-5">
              ...
              <p className="mt-1 text-sm text-muted-fg">
                {r.restaurantName} • order {r.orderRef} • {new Date(r.orderedAt).toLocaleDateString()}
              </p>
...
function SummaryCard({ summary, loading }: { summary: FoodReviewSummary | null; loading: boolean }) {
  return (
    <Card className="p-5">

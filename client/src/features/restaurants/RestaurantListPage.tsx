    ...
    <Link to={`/app/restaurants/${r.id}`}>
      <Card className="h-full overflow-hidden transition-shadow hover:shadow-md">
        <div className="flex h-36 items-center justify-center bg-gradient-to-br from-rose-500/15 to-rose-500/0">
          <UtensilsCrossed className="size-10 text-rose-500/60" />
        </div>
        <div className="p-4">
          <div className="flex items-start justify-between gap-2">
            ...
            {r.rating && (
              <span className="inline-flex items-center gap-1 text-xs font-medium">
                <Star className="size-3.5 fill-amber-400 text-amber-400" /> {r.rating.average.toFixed(1)}
            ...
            <div>
              {r.fromPrice != null ? (
                <>
                  ...
                  <span className="font-display text-lg font-bold">रू {r.fromPrice.toLocaleString()}</span>
                </>
              ) : (
